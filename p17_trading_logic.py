            ...
            while True:
                ...
                if trend_score < -0.15:  # 下降トレンドで買い
                    if usdt_balance >= self.min_trade_amount:
                        ...
                        btc_qty = math.ceil((buy_amount_usdt / current_price) * 100000) / 100000
                ...
                elif trend_score > 0.15:  # 上昇トレンドで売り
                    if btc_holding > 0:
                        ...
                        if btc_value >= self.min_trade_amount:
                            ...
                            if trend_strength > 0.3:  # 強い上昇トレンド
                                sell_amount_usdt = min(btc_value, self.max_trade_amount)
                            else:
                                sell_amount_usdt = min(btc_value, self.max_trade_amount * 0.7)
                            
                            # 最小取引額以上になるように調整
                            sell_amount_usdt = max(sell_amount_usdt, self.min_trade_amount)
                            
                            btc_qty = math.ceil((sell_amount_usdt / current_price) * 100000) / 100000
